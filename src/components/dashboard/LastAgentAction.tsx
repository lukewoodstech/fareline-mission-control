...
export default function LastAgentAction({ action }: LastAgentActionProps) {
  const prevIdRef = useRef(action.id);
  const [isNew, setIsNew] = useState(false);

  useEffect(() => {
    if (action.id !== prevIdRef.current) {
      prevIdRef.current = action.id;
      setIsNew(true);
  ...
  return (
    <Card className={`h-full transition-all duration-500 ${
      isNew ? "glow-primary ring-1 ring-primary/30" : "glow-primary"
    }`}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
            <Zap className={`h-4 w-4 text-primary ${isNew ? "animate-bounce" : ""}`} />
            Last Agent Action
          </CardTitle>
      ...
      <CardContent>
        <div key={action.id} className={isNew ? "animate-slide-up" : ""}>
          <div className="space-y-3">
            <div className="flex items-start justify-between gap-3">
              <p className="font-semibold text-sm leading-snug">{action.summary}</p>
              {action.smsSent && (
                <Badge variant="accent" className={`shrink-0 text-xs ${isNew ? "animate-sms-pop" : ""}`}>
